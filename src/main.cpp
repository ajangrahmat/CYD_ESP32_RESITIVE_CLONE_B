#include <Arduino.h>
#include <WiFi.h>
#include <WiFiClientSecure.h>
#include <HTTPClient.h>
#include <ArduinoJson.h>
#include <TFT_eSPI.h>
#include <time.h>
#include <SPI.h>
#include <SD.h>

// =======================
// KONFIGURASI
// =======================
#define WIFI_SSID     "isi nama wifi kamu"
#define WIFI_PASS     "isi password wifi kamu"
#define REFRESH_MS    300000
#define TIME_UPDATE_MS 1000

#define TIMEZONE_OFFSET 7 * 3600
#define NTP_SERVER1 "pool.ntp.org"
#define NTP_SERVER2 "time.nist.gov"

// ✅ HAPUS custom HSPI — pakai pin default SD pada CYD
#define SD_CS     5

TFT_eSPI tft;

float currentRate = 0;
float prevRate    = 0;
String lastUpdate = "--:--";
float history[100];
int historyCount = 0;

char timeStr[9] = "00:00:00";
char dateStr[24] = "Day, 00 Jan 0000";
bool timeSynced = false;

bool sdOk = false;
unsigned long lastSDsave = 0;

WiFiClientSecure client;
// ✅ HAPUS: SPIClass sdSPI(HSPI); — tidak diperlukan

// =======================
// DEBUG
// =======================
void debug(const char* msg) {
  Serial.print(millis()); Serial.print(": "); Serial.println(msg);
}
void debug(String msg) {
  Serial.print(millis()); Serial.print(": "); Serial.println(msg);
}

// =======================
// SD CARD — FIXED
// =======================
void initSD() {
  debug("Init SD card...");

  // ✅ Gunakan SD.begin() standar, BUKAN custom HSPI
  if (!SD.begin(SD_CS)) {
    sdOk = false;
    debug("SD Card GAGAL");
    return;
  }
  sdOk = true;
  uint64_t cardSize = SD.cardSize() / (1024 * 1024);
  debug("SD Card OK, Size: " + String((int)cardSize) + " MB");
}

void saveHistoryToSD() {
  if (!sdOk) return;
  File file = SD.open("/rate_history.csv", FILE_WRITE);
  if (!file) {
    debug("Gagal buka file untuk write");
    return;
  }
  for (int i = 0; i < historyCount; i++) {
    file.print(history[i]);
    if (i < historyCount - 1) file.print(",");
  }
  file.println();
  file.close();
  debug("History saved to SD");
}

void loadHistoryFromSD() {
  if (!sdOk) return;
  debug("Loading history from SD...");
  File file = SD.open("/rate_history.csv", FILE_READ);
  if (!file) {
    debug("No history file found");
    return;
  }
  String data = file.readString();
  file.close();
  data.trim();
  if (data.length() == 0) return;

  int idx = 0;
  historyCount = 0;
  while (idx < (int)data.length() && historyCount < 100) {
    int comma = data.indexOf(',', idx);
    if (comma == -1) comma = data.length();
    float val = data.substring(idx, comma).toFloat();
    if (val > 0) history[historyCount++] = val;
    idx = comma + 1;
    if (comma >= (int)data.length()) break;
  }
  debug("Loaded " + String(historyCount) + " data points");
}

// =======================
// WIFI
// =======================
void connectWiFi() {
  debug("Connecting WiFi...");
  tft.fillScreen(TFT_BLACK);
  tft.setTextColor(TFT_CYAN);
  tft.setTextSize(2);
  tft.setCursor(80, 100);
  tft.print("Connecting WiFi");

  WiFi.begin(WIFI_SSID, WIFI_PASS);
  int dot = 0;
  while (WiFi.status() != WL_CONNECTED) {
    tft.setCursor(120 + (dot * 10), 130);
    tft.print(".");
    dot = (dot + 1) % 6;
    if (dot == 0) tft.fillRect(120, 130, 60, 20, TFT_BLACK);
    delay(500);
  }
  debug("WiFi connected: " + WiFi.localIP().toString());
  tft.fillScreen(TFT_BLACK);
  tft.setTextColor(TFT_GREEN);
  tft.setCursor(100, 110);
  tft.print("WiFi Connected!");
  delay(1000);
}

// =======================
// NTP
// =======================
void initNTP() {
  debug("Init NTP...");
  configTime(TIMEZONE_OFFSET, 0, NTP_SERVER1, NTP_SERVER2);
  struct tm timeinfo;
  timeSynced = false;
  unsigned long start = millis();
  while (millis() - start < 10000) {
    if (getLocalTime(&timeinfo)) {
      timeSynced = true;
      break;
    }
    delay(500);
  }
  debug(timeSynced ? "NTP sync OK" : "NTP sync FAILED");
}

void updateTimeDisplay() {
  if (!timeSynced) return;
  struct tm timeinfo;
  if (!getLocalTime(&timeinfo)) return;

  char newTime[9];
  strftime(newTime, sizeof(newTime), "%H:%M:%S", &timeinfo);
  if (strcmp(timeStr, newTime) != 0) {
    strcpy(timeStr, newTime);
    tft.setTextColor(TFT_WHITE, TFT_BLACK);
    tft.setTextSize(1);
    tft.setCursor(260, 44);
    tft.print("        ");
    tft.setCursor(260, 44);
    tft.print(timeStr);
  }

  char newDate[24];
  strftime(newDate, sizeof(newDate), "%A, %d %B %Y", &timeinfo);
  if (strcmp(dateStr, newDate) != 0) {
    strcpy(dateStr, newDate);
    tft.setTextColor(TFT_YELLOW, TFT_BLACK);
    tft.setTextSize(1);
    tft.setCursor(10, 28);
    tft.print("                                ");
    tft.setCursor(10, 28);
    tft.print(dateStr);
  }
}

// =======================
// API
// =======================
float fetchRate() {
  debug("Fetching rate...");
  client.setInsecure();
  HTTPClient http;
  http.begin(client, "https://open.er-api.com/v6/latest/USD");
  http.setTimeout(10000); // ✅ timeout 10 detik
  int httpCode = http.GET();
  if (httpCode != 200) {
    http.end();
    debug("HTTP error: " + String(httpCode));
    return 0;
  }
  String payload = http.getString();
  http.end();

  JsonDocument doc;
  DeserializationError error = deserializeJson(doc, payload);
  if (error) {
    debug("JSON parse error: " + String(error.c_str()));
    return 0;
  }
  float rate = doc["rates"]["IDR"].as<float>();
  debug("Rate: " + String(rate));
  return rate;
}

void addHistory(float value) {
  if (historyCount < 100) {
    history[historyCount++] = value;
  } else {
    for (int i = 0; i < 99; i++) history[i] = history[i+1];
    history[99] = value;
  }
}

// =======================
// GRAFIK
// =======================
void drawMiniGraph() {
  if (historyCount < 2) return;
  int gx = 20, gy = 160, gw = 280, gh = 40;
  float minV = history[0], maxV = history[0];
  for (int i = 1; i < historyCount; i++) {
    if (history[i] < minV) minV = history[i];
    if (history[i] > maxV) maxV = history[i];
  }
  if ((maxV - minV) < 50) { maxV += 25; minV -= 25; }
  tft.fillRect(gx, gy, gw, gh, TFT_BLACK);
  float stepX = (float)gw / (historyCount - 1);
  for (int i = 0; i < historyCount - 1; i++) {
    int x1 = gx + (int)(i * stepX);
    int x2 = gx + (int)((i+1) * stepX);
    int y1 = gy + gh - map(history[i] * 100, minV * 100, maxV * 100, 0, gh);
    int y2 = gy + gh - map(history[i+1] * 100, minV * 100, maxV * 100, 0, gh);
    uint16_t color = (history[i+1] >= history[i]) ? TFT_RED : TFT_GREEN;
    tft.drawLine(x1, y1, x2, y2, color);
    tft.fillCircle(x2, y2, 2, color);
  }
}

// =======================
// DRAW UI
// =======================
void drawUI() {
  debug("Drawing UI...");
  tft.fillScreen(TFT_BLACK);

  tft.setTextColor(TFT_CYAN);
  tft.setTextSize(2);
  tft.setCursor(10, 8);
  tft.print("USD  ->  IDR");

  tft.setTextSize(1);
  tft.setTextColor(WiFi.status() == WL_CONNECTED ? TFT_GREEN : TFT_RED);
  tft.setCursor(250, 8);
  tft.print(WiFi.status() == WL_CONNECTED ? "WiFi OK" : "No WiFi");

  tft.setTextColor(sdOk ? TFT_GREEN : TFT_RED);
  tft.setCursor(180, 8);
  tft.print(sdOk ? "SD OK" : "SD Fail");

  tft.setTextColor(TFT_YELLOW);
  tft.setCursor(10, 28);
  tft.print(dateStr);
  tft.setTextColor(TFT_WHITE);
  tft.setCursor(260, 44);
  tft.print(timeStr);

  tft.drawFastHLine(0, 58, 320, TFT_DARKGREY);

  tft.setTextColor(TFT_WHITE);
  tft.setTextSize(4);
  tft.setCursor(20, 85);
  tft.printf("Rp %0.0f", currentRate);

  tft.setTextSize(2);
  if (prevRate > 0 && prevRate != currentRate) {
    float diff = currentRate - prevRate;
    tft.setTextColor(diff >= 0 ? TFT_RED : TFT_GREEN);
    tft.setCursor(20, 135);
    if (diff >= 0) tft.printf("^ +%0.0f", diff);
    else           tft.printf("v %0.0f", diff);
  }

  drawMiniGraph();

  tft.drawFastHLine(0, 215, 320, TFT_DARKGREY);
  tft.setTextColor(TFT_DARKGREY);
  tft.setTextSize(1);
  tft.setCursor(10, 225);
  tft.print("Source: open.er-api.com");
  tft.setCursor(220, 225);
  tft.print("Upd: " + lastUpdate);
}

// =======================
// SETUP
// =======================
void setup() {
  Serial.begin(115200);
  delay(500);
  debug("=== STARTING ===");

  tft.init();
  tft.setRotation(0);
  tft.fillScreen(TFT_BLACK);
  debug("TFT initialized");

  connectWiFi();
  initNTP();

  // ✅ Delay kecil sebelum init SD agar SPI TFT stabil dulu
  delay(200);
  initSD();
  if (sdOk) loadHistoryFromSD();

  float rate = fetchRate();
  if (rate > 0) {
    currentRate = rate;
    prevRate = rate;
    addHistory(rate);
  } else {
    currentRate = 16000;
    prevRate = 16000;
    addHistory(16000);
  }

  struct tm timeinfo;
  if (timeSynced && getLocalTime(&timeinfo)) {
    strftime(timeStr, sizeof(timeStr), "%H:%M:%S", &timeinfo);
    strftime(dateStr, sizeof(dateStr), "%A, %d %B %Y", &timeinfo);
  } else {
    strcpy(timeStr, "00:00:00");
    strcpy(dateStr, "Unknown, 01 Jan 2000");
  }

  drawUI();
  debug("Setup complete");
}

// =======================
// LOOP
// =======================
void loop() {
  static unsigned long lastRateTime = 0;
  static unsigned long lastTimeUpdate = 0;
  unsigned long now = millis();

  if (now - lastTimeUpdate >= TIME_UPDATE_MS) {
    lastTimeUpdate = now;
    updateTimeDisplay();
  }

  if (now - lastRateTime >= REFRESH_MS) {
    lastRateTime = now;
    if (WiFi.status() != WL_CONNECTED) {
      WiFi.disconnect();
      delay(1000);
      WiFi.reconnect();
    }
    prevRate = currentRate;
    float rate = fetchRate();
    if (rate > 0) {
      currentRate = rate;
      addHistory(rate);
      struct tm timeinfo;
      if (timeSynced && getLocalTime(&timeinfo)) {
        char buf[6];
        strftime(buf, sizeof(buf), "%H:%M", &timeinfo);
        lastUpdate = String(buf);
      } else {
        lastUpdate = "--:--";
      }
      drawUI();
    }
  }

  if (sdOk && (now - lastSDsave >= 60000)) {
    lastSDsave = now;
    saveHistoryToSD();
  }

  delay(100);
}